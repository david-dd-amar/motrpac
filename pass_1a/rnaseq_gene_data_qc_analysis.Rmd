...
dmaqc_metadata_path = 
  "/Users/David/Desktop/MoTrPAC/data/april_2019/DMAQC_Transfer_Pass_1A.6M_1/3-Data_Sets/"
...
rownames(specimen_data) = as.character(specimen_data$labelid)
...
```{r, fig.align="center",out.height='70%',out.width='70%',message=FALSE,warning=FALSE,eval=TRUE}
...
break_label_id<-function(x){
  if(nchar(x)!=11){stop("Input size is not 11 char long")}
  if(is.na(as.numeric(x))){stop("Input is not a number")}
  bid = substr(x,1,5)
  tp = substr(x,6,7)
  tissue = substr(x,8,9)
  aliq = substr(x,10,11)
  return(list(bid=bid,tp=tp,tissue_code=tissue,aliq=aliq))
}

parse_shocktime <-function(x){
  if(length(x)==0||nchar(x)==0||is.na(x)||is.null(x)){return(0)}
  arr = strsplit(x,split = ":")[[1]]
  num = as.numeric(arr[1])*60 + as.numeric(arr[2])
  # print(num)
  return(num)
}

# helper function to get the metadata (all non-assay data in one place)
# specimen data is optional (we want it for time-related variables for now)
get_sample_metadata<-function(labelids,assay_data,animal_data,specimen_data=NULL){
  labelids = as.character(labelids)
  labelid2pids = as.character(assay_data$PID)
  names(labelid2pids) = as.character(assay_data[,1])
  if(!is.null(specimen_data)){
      labelid2pids2 = as.character(specimen_data$pid)
      names(labelid2pids2) = as.character(specimen_data$labelid)
      labelid2pids = c(labelid2pids,labelid2pids2)
  }
  m = cbind(assay_data[labelids,],animal_data[labelid2pids[labelids],])
  if(!is.null(specimen_data)){m = cbind(m,specimen_data[labelids,])}
  return(m)
}

# # A hack - As of May 2019, the vial ids in the RNA-seq metadata
# # do not match the label ids from DMAQC (poweder vs. non-powder tissue). 
# # We therefore label the data by concatenating the bid,
# # tp, and the tissue name
# labels1 = paste(rnaseq_meta$BID,tolower(rnaseq_meta$Tissue),sep=";")
# names(labels1) = rnaseq_meta$vial_label
# labels2 = paste(specimen_data$bid,tolower(specimen_data$sampletypedescription),sep=";")
# names(labels2) = specimen_data$labelid
# # check samples with no duplication at the label level
# shared_samples = intersect(labels1,labels2)
# rownames(specimen_data) = labels2
# rownames(rnaseq_meta) = labels1

labelids = rnaseq_meta$vial_label
test = get_sample_metadata(labelids,rnaseq_meta,animal_metadata)
...
for(i in 1:nrow(rnaseq_meta_batchs)){
  ...
  curr_pcax = curr_pca$x[,1:10]
  curr_meta = get_sample_metadata(curr_samples,rnaseq_meta,animal_metadata)
  curr_meta = curr_meta[,c(assay_cols_for_qc_analysis,
                           animal_data_cols_for_qc_analysis)]
  curr_meta$howlongshock = sapply(curr_meta$howlongshock,parse_shocktime)
  corrs = cor(curr_pcax,curr_meta)
  corrplot(corrs)
}
...
2. Automatic flow that links PCs to QC scores
