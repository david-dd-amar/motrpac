...
# Check the freeze times of the samples
...
colnames(tissue_log_ps) = c(
  "collect_to_freeze","death_to_collect"
)
...
We tested two ways to normalize the count data: (1) FPKM, and (2) factor normalized counts with and without variance stabilizing transformations (to accound for gene dispersion). 
...
animal_data_cols_for_qc_analysis = c(
  ...
  "calculated.variables.time_complete_to_death_min",
  "calculated.variables.time_death_to_collect_min",
...
for(i in 1:nrow(rnaseq_meta_batchs)){
  ...
  colnames(corrs2) = sapply(colnames(corrs2),function(x)strsplit(x,split='\\.')[[1]][3])
...
assay_cols_for_qc_analysis = c("pct_GC","pct_globin","pct_multimapped")
# Animal information to correlate with genes/pcs
# FUTURE WORK: add site id and batch
animal_data_cols_for_qc_analysis = c(
...
for(i in 1:nrow(rnaseq_meta_batchs)){
  ...
  curr_meta = cbind(curr_meta1,curr_meta2)
  pi1_meta = c()
  nsamp=2000
  pvals = list()
  for(j in 1:ncol(curr_meta)){
    samp = sample(1:nrow(curr_data))[1:nsamp]
    res = apply(curr_data[samp,],1,linear_association_analysis,y=curr_meta[,j])
    if(class(res)=="matrix"){
      res_ps = res[1,]
    }
    else{
      res_ps = sapply(res, function(x)x[1])
    }
    hist(res_ps,main = paste(colnames(curr_meta)[j],curr_tissue,sep=","))
    pi1 = 1 - propTrueNull(res_ps,nbins = 20)
    pi1_meta[colnames(curr_meta)[j]] = pi1
    pvals[[colnames(curr_meta)[j]]] = res_ps
  }
  
  dataset2pi1s = rbind(dataset2pi1s,pi1_meta)
  currname = paste(curr_tissue,curr_site,sep=",")
  rownames(dataset2pi1s)[nrow(dataset2pi1s)] = currname
  dataset2pvals[[currname]] = pvals
  # Add some plots for selected tissues
  if(grepl("white adipose",curr_tissue) || 
     grepl("heart|gastro",curr_tissue,ignore.case = T)){
  }
...
## Create the datasets and save
```{r,eval=FALSE}
# Prepare the data for the analysis
Z_cols = c("RIN","reads","pct_GC","pct_rRNA","pct_globin",
                      "pct_umi_dup","median_5_3_bias","pct_trimmed_bases",
                      "pct_multimapped","pct_GC","pct_chrM")
clinical_cols = c("animal.registration.sex","animal.key.timepoint",
                  "animal.key.is_control","acute.test.distance",
                  "acute.test.weight",
                  "calculated.variables.frozetime_after_acute",
...
for(i in 1:nrow(rnaseq_meta_batchs)){
  ...
  non_control_inds = (!curr_meta2$animal.key.is_control) | 
    (curr_meta2$animal.key.is_control & curr_meta2$animal.key.timepoint == 
       min(curr_meta2$animal.key.timepoint))
...
```{r,fig.align="center",out.height='40%',out.width='40%',eval=FALSE,warning=FALSE}
library(mediation)
...
for(curr_name in names(rnaseq_data_for_difftests)){
  ...
  for(tp in unique(y)){
    ...
    colnames(ttest_res) = c("mean1","mean2","p","fchange")
    curr_entry_name = paste(curr_name,tp,sep=",")
    t_tests_results[[curr_entry_name]] = ttest_res
  }
}
...
for(curr_name in names(rnaseq_data_for_difftests)){
  ...
  regression_res = tissue2diff_analysis_results[[curr_name]]
  regression_res = as.data.frame(regression_res$diff_res)
  p1 = regression_res$`pval;animal.registration.sex`
  p2 = regression_res$`pval;time.quad`
  curr_genes = rownames(regression_res)[
    p.adjust(p1,method="fdr") < 0.1 &
      p.adjust(p2,method="fdr") < 0.1
  ]
  
  for(tp in unique(y)){
    curr_entry_name = paste(curr_name,tp,sep=",")
    x = curr_data[,y==tp]
    ttest_res = t_tests_results[[curr_entry_name]]
    fchange = ttest_res[,"fchange"]
...
system(paste("~/google-cloud-sdk/bin/gsutil", 
             ...
             "gs://bic_data_analysis/pass1a/rnaseq/"))
system("rm tissue2diff_ttest_and_mediation.RData")
...
# load precomputed data and results
tissue2diff_analysis_results = load_from_bucket(
  "tissue2diff_analysis_results.RData",
  "gs://bic_data_analysis/pass1a/rnaseq/"
)[[1]]
one_way_results = load_from_bucket(
  "tissue2diff_analysis_results_simple_univariate.RData",
  "gs://bic_data_analysis/pass1a/rnaseq/"
)
tissue2diff_analysis_results_anova = one_way_results[[
  "tissue2diff_analysis_results_anova"
]]
tissue2diff_analysis_results_kw = one_way_results[[
  "tissue2diff_analysis_results_kw"
]]
...
mediation_results = pairwise_tests_results[["mediation_results"]]
...
Here we use empirical Bayes methods to quantify the number of differential genes in each analysis.
...
corrplot(cor(all_ttest_pvals),tl.cex = 0.5)
corrplot(cor(all_ttest_fcs),tl.cex = 0.5,order="hclust")

hist(all_ttest_pvals)
colSums(all_ttest_pvals<pval_thr)

# Network prop or simple distances
...
res_mat = res_mat[names(V(g)),]

prop_res = dRWRpipeline(res_mat,g,normalise="none",
                restart=0.5, parallel=FALSE,method="direct")
prop_res1 = as.matrix(prop_res$ratio)
rownames(prop_res1) = colnames(res_mat)
colnames(prop_res1) = colnames(res_mat)
dim(prop_res1)
hist(prop_res1)
corrplot(prop_res1,tl.cex = 0.5,order="hclust",is.corr=F)

# Direct computation of propagations or distances
prop_matrix = c()
distance_matrix = c()
prop_results = dRWR(g,normalise = "none",setSeeds = res_mat,restart = 0.5)
for(nn1 in colnames(res_mat)){
  v = as.matrix(res_mat[,nn1])
  print(table(v))
  rownames(v) = rownames(res_mat)
  curr_prop = dRWR(g,normalise = "none",setSeeds = v,restart = 0.5)
  curr_prop = as.matrix(curr_prop)
  rownames(curr_prop) = names(V(g))
  prop_results = cbind(prop_results,curr_prop[,1])
  new_scores = c()
  for(nn2 in colnames(res_mat)){
    set2 = rownames(res_mat)[res_mat[,nn2]>0]
    new_scores[nn2] = sum(curr_prop[set2,1])
  }
  prop_matrix = rbind(prop_matrix,new_scores)
  rownames(prop_matrix)[nrow(prop_matrix)] = nn1
}
corrplot(prop_matrix,is.corr = F)


# # An alternative: create an integrated graph first and then propagate
# merged_net = igraph::as_edgelist(g)
# for(nn in colnames(res_mat)){
#   curr_genes = rownames(res_mat)[res_mat[,nn]>0]
#   curr_edges = cbind(rep(nn,length(curr_genes)),curr_genes)
...
# for(nn in names(res_mat)){seeds[nn,nn]=sum(res_mat[,nn])}
# prop_res = dRWR(merged_g,"laplacian",seeds,
#                 normalise.affinity.matrix = "quantile",
#                 restart = 0.5)
# prop_res = as.matrix(prop_res)
# colnames(prop_res) = colnames(seeds)
# rownames(prop_res) = rownames(seeds)
# prop_res = prop_res[colnames(res_mat),colnames(res_mat)]
# diag(prop_res) = 0
# for(i in 1:nrow(prop_res)){
# 	prop_res[i,] = prop_res[i,]/(sum(prop_res[i,]))
# }
# 
# corrplot(prop_res,is.corr = F,tl.cex = 0.5,order="hclust")
# corrplot(prop_res,is.corr = F)

```

We next extract the fold change and p-value matrices and compare the tissues, with and without network propagation.

## Enrichment analysis and interpretation

```{r}
library(org.Rn.eg.db)
res = t_tests_results$`brown adipose,Stanford,4`
geneset = rownames(res)[p.adjust(res[,3])<0.1]
topgo_res = run_topgo_enrichment_fisher(geneset,rownames(res),
    mapping = "org.Rn.eg.db",ID="ensembl")
extract_top_go_results(topgo_res,qval = 0.01,maxsize = 200)

med_res = mediation_results$`gastrocnemius,MSSM,4`
colnames(med_res)
med_res[,2] = pmax(med_res[,2],0.0001)
med_res[,7] = pmax(med_res[,7],0.0001)
weight_assoc_genes = p.adjust(med_res[,2],method="fdr")<0.1
sex_assoc_genes = p.adjust(med_res[,7],method="fdr")<0.1
table(weight_assoc_genes,sex_assoc_genes)
table(weight_assoc_genes)

geneset = rownames(med_res)[weight_assoc_genes]
geneset = names(which(sex_assoc_genes))
topgo_res = run_topgo_enrichment_fisher(
  geneset,rownames(res),mapping = "org.Rn.eg.db",ID="ensembl")
extract_top_go_results(topgo_res,qval = 1,maxsize = 2000)

dataset = rnaseq_data_for_difftests$`gastrocnemius,MSSM`
X = dataset$fpkms[geneset,]
cols = as.factor(dataset$dmaqc_meta$animal.key.timepoint)
boxplot(X[2,]~dataset$dmaqc_meta$animal.registration.sex)
inds = dataset$dmaqc_meta$animal.registration.sex==1
plot(X[2,inds],dataset$dmaqc_meta$acute.test.weight[inds])
