    ...
    for(another_dataset in names(metabolomics_raw_datasets_skip)){
      ...
      if(curr_site == another_d_site &&
        ...
        length(another_shared_vials)==length(curr_vials)
          ...
          # make sure that the vials, types, and order are te same
          if(length(shared_meta_samples) < 1 ||
            !all(curr_meta[shared_meta_samples,1:3]==another_meta[shared_meta_samples,1:3])){
            next
          }
          try({
            newd = merge_named_and_unnamed_metabolomics_datasets(
              newd,another_d,strict = F) # no strict merge
...
pvals_tar = sapply(meta_analysis_stats,function(x)x$re_model1$pval)
