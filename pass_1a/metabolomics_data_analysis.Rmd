...
for(tissue in tissues){
  ...
  rownames_arr = t(sapply(rownames(tissue_m),function(x)strsplit(x,split=",")[[1]]))
  tissue_m = cbind(rownames_arr[,1],rownames_arr[,3],tissue_m)
  ...
  for(nn in rownames(tissue_m)){
    ...
    tar_mean = mean(sapply(currmet_info_tar,function(x)x[3]))
    untar_cov = mean(sapply(currmet_info_untar,function(x)x[4]))
    tar_cov = mean(sapply(currmet_info_tar,function(x)x[4]))
    tar_nas = mean(sapply(currmet_info_tar,function(x)x[1]))
    untar_nas = mean(sapply(currmet_info_untar,function(x)x[1]))
    met_stats = rbind(met_stats,
                      c(tar_mean,untar_stand_mean,tar_cov,untar_cov,tar_nas,untar_nas))
  }
  ...
  for(j in 2:ncol(tissue_m)){
    tissue_m[[j]] = as.numeric(as.character(tissue_m))
  }
  write.table()

  # Create the metabolite-level summary per tissue
