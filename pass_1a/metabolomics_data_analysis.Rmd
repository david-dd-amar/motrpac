...
rdata_objects = sapply(rdata_files,load_from_bucket,bucket=metabolomics_data_bucket,delete=F)

# Separate the datasets by site, tissue, and type (metadata or not)
raw_abundance_datasets = lapply(rdata_objects,function(x,y)x[grepl(y,names(x))],y="compounds")
sample_metadata = lapply(rdata_objects,function(x,y)x[grepl(y,names(x))],y="sampleme")
get_tissue_from_dataset_name<-function(x){return(strsplit(x,split="_")[[1]][2])}
raw_abundance_datasets_tissues = lapply(raw_abundance_datasets,
                                    function(x)sapply(names(x),get_tissue_from_dataset_name))
...
for(site in names(raw_abundance_datasets)){
  is_untargeted = site == "broad"
  for(tissue in names(raw_abundance_datasets[[site]])){
    d = raw_abundance_datasets[[site]][[tissue]]
    sample_meta = sample_metadata[[site]][[tissue]]
    tissue_name = get_tissue_from_dataset_name(tissue)
    if(is_untargeted){
      row_annot = d[,1:8]
      d = d[-c(1:8)]
    }
    ...
    metabolomics_parsed_datasets[[obj_name]] = obj
    # if(ncol(control_data)>0){hist(cor(d,control_data),main=obj_name)}
  }
}
```
...
for(nn in names(metabolomics_parsed_datasets)){
  ...
  cor_matrices[[nn]] = cor(pcax)
}
sapply(cor_matrices,dim)
library('evolqg');library(corrplot)
mantel_tests= MatrixCor(cor_matrices)
diag(mantel_tests)=1
corrplot(mantel_tests,type="lower")
