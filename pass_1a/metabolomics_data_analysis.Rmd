...
# normdiffs_is_untar = grepl("untar",rownames(normdiffs))
# corrplot(normdiffs[normdiffs_is_untar,],is.corr = F,tl.cex = 0.7)
# corrplot(normdiffs[!normdiffs_is_untar,],is.corr = F,tl.cex = 0.7)
...
for(pair_name in names(norm_method2comparison_results)){
  ...
  single_metabolite_de = norm_method2comparison_results$single_metabolite_de
...
for(pair_name in names(norm_method2comparison_results)){
  ...
  all_untar_pvals = cbind(all_untar_pvals,pvals_untar)
  ...
  rho = cor(pvals_tar,pvals_untar,method = "pearson")
  rhop = cor.test(pvals_tar,pvals_untar,method = "pearson")$p.value
  print(
    ggplot(df, aes(x=targeted, y=untargeted,
                 shape=significant_diff, color=significant_in)) +
      geom_point() +
      ggtitle(paste(pair_name,"-log10 p-values, rho=:",format(rho,digits=2),
  ...
  rho = cor(betas_untar,betas_tar,method = "pearson")
  rhop = cor.test(betas_untar,betas_tar,method = "pearson")$p.value
  print(
    ggplot(df, aes(x=targeted, y=untargeted,
                 shape=significant_diff, color=significant_in)) +
      geom_point() +
    ggtitle(paste(pair_name,"effect sizes, rho=:",format(rho,digits=2),
                  "(p=",format(rhop,digits=3),")"))
  )
}
...
single_metabolite_corrs = norm_method2comparison_results$`log2,imp;log2,imp`
